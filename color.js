...
function createColorGroupPreview(groupKey, groupState, darkMode) {
    ...
    const swatchesHTML = groupState.variants.map((variant, index) => {
        ...
        return `
            <div class="flex flex-col w-[120px] mb-8">
                <div class="h-24 w-[120px] flex items-center justify-center bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded">
                    <div class="w-16 h-16 rounded-full" style="background-color: ${variant.value}; box-shadow: inset 0 0 0 1px rgba(0,0,0,0.1);"></div>
                </div>
                <div class="mt-2 text-left">
                    <span class="text-xs text-zinc-500 dark:text-zinc-400 font-mono break-all block">${labelHTML}</span>
                </div>
            </div>
        `;
    }).join('');
    ...
    return `
        <div class="mb-20">
            <h3 class="pst-preview-heading mb-4 pb-2" ${headingStyle}>${groupName}</h3>
            <div class="flex flex-row flex-wrap gap-x-8 gap-y-12">
                ${swatchesHTML}
            </div>
        </div>
    `;
...
function createGradientPreview(gradientState, darkMode) {
    ...
    const headingStyle = darkMode ? 'style="color: #888888;" font-family: var(--pst-shell-font);' : 'font-family: var(--pst-shell-font);';
    ...
    const gradientSwatches = gradientState.variants.map((grad, index) => {
        ...
        return `
            <div class="flex flex-col w-[120px] mb-8">
                <div class="h-24 w-[120px] flex items-center justify-center bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded">
                    <div class="w-16 h-16 rounded-full" style="background: ${generateGradientCSS(grad)}; box-shadow: inset 0 0 0 1px rgba(0,0,0,0.1);"></div>
                </div>
                <div class="mt-2 text-left">
                    <span class="text-xs text-zinc-500 dark:text-zinc-400 font-mono break-all block">${tokenName}</span>
                </div>
            </div>
        `;
    }).join('');
    ...
    return `
        <div class="mb-20">
            <h3 class="pst-preview-heading mb-4 pb-2" ${headingStyle}>Gradients</h3>
            <div class="flex flex-row flex-wrap gap-x-8 gap-y-12">${gradientSwatches}</div>
        </div>`;
}
...
window.colorView = {
    ...
    getPreviewHTML: (state) => {
        ...
        const content = (groupsHTML + gradientsHTML) || '<div class="pst-system-feedback">No color groups defined.</div>';
        return content;
    },
