...
const iconView = {
    ...
    getPreviewHTML: (state) => {
        ...
        if (!selectedLibraryKey || selectedLibraryKey === 'none') {
            return '<div class="pst-system-feedback">Select an icon library from the controls to see a preview.</div>';
        }
        ...
        const iconSamplesHTML = library.samples.slice(0, 32).map(iconName => {
            ...
            return `
                <div class="flex flex-col items-start text-left">
                    <div class="flex flex-col items-start space-y-6 p-4 border border-zinc-200 dark:border-zinc-800 rounded bg-zinc-50 dark:bg-zinc-900 w-full">
                        ${largeIcon}
                        ${mediumIcon}
                        ${smallIcon}
                    </div>
                    <span class="text-xs text-zinc-500 dark:text-zinc-400 mt-2 font-mono break-all block">${iconName}</span>
                </div>
            `;
        }).join('');
        ...
        return `
            <div class="mb-20">
                ...
                <div class="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-x-8 gap-y-16">
                    ${iconSamplesHTML}
                </div>
            </div>
        `;
