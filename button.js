    ...
    getPreviewHTML: (state) => {
        ...
        const variantsHTML = buttonState.variants.map((variant, index) => {
            ...
            if (variant.name === 'Pair') {
                ...
                const pairPreviews = sizes.map(size => `
                    <div class="flex flex-col items-start mb-6">
                        <div class="btn-pair flex items-center mb-2">
                            <button class="btn btn-${size} ${leftVariantClass}">${labelLeft}</button>
                            <button class="btn btn-${size} ${rightVariantClass}">${labelRight}</button>
                        </div>
                        <span class="text-xs text-zinc-500 dark:text-zinc-400 font-mono whitespace-nowrap">.btn-pair (${size})</span>
                    </div>
                `).join('');
                ...
                return `
                    <div class="mb-20">
                        ...
                        <div class="w-full">${pairPreviews}</div>
                    </div>
                `;
            }
            ...
            const previews = sizes.map(size => {
                ...
                return `
                    <div class="flex flex-col items-start mb-6">
                        <div class="flex items-center space-x-4 mb-2">${labelOnly}${labelIcon}${iconOnly}</div>
                        <span class="text-xs text-zinc-500 dark:text-zinc-400 font-mono">${sizeTokenName}</span>
                    </div>
                `;
            }).join('');
