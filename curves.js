const curvesView = {
    ...
    getPreviewHTML: (state) => {
        if (!state.curves || state.curves.variants.length === 0) {
            return '<div class="pst-system-feedback">No curve or elevation settings defined.</div>';
        }

        const curvesVariantsHTML = state.curves.variants.map((variant, index) => {
            const tokenName = `--border-radius-${index + 1}`;
            return `
                <div class="flex flex-col w-[160px] items-start mb-6">
                    <div class="flex items-center justify-start h-[160px] w-full bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded p-4">
                        <div class="w-full h-full" style="background-color: none; border: 2px solid #9ca3af; border-radius: ${variant.radius}px;"></div>
                    </div>
                    <div class="pt-2 text-left">
                        <span class="text-xs text-zinc-500 dark:text-zinc-400 font-mono break-all">${tokenName}</span>
                    </div>
                </div>
            `;
        }).join('');
        ...
        const elevationVariantsHTML = state.curves.elevation.variants.map((variant, index) => {
            ...
            return `
                <div class="flex flex-col w-[160px] items-start mb-12">
                    <div class="flex items-center justify-start h-[160px] w-full bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded p-4">
                        <div class="w-full h-full" style="background-color: ${testBgColor}; border-radius: ${maxRadius}px; box-shadow: ${shadowStyle};"></div>
                    </div>
                    <div class="pt-2 text-left">
                        <span class="text-xs text-zinc-500 dark:text-zinc-400 font-mono break-all">${tokenName}</span>
                    </div>
